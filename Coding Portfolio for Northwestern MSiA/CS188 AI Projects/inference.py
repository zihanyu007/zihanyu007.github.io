...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    variableDomainsDict[PAC] = [(i, j) for i in range(0, X_RANGE) for j in range(0, Y_RANGE)]
    variableDomainsDict[GHOST0] = [(i, j) for i in range(0, X_RANGE) for j in range(0, Y_RANGE)]
    variableDomainsDict[GHOST1] = [(i, j) for i in range(0, X_RANGE) for j in range(0, Y_RANGE)]
    variableDomainsDict[OBS0] = range(0,X_RANGE+Y_RANGE+MAX_NOISE-1)
    variableDomainsDict[OBS1] = range(0,X_RANGE+Y_RANGE+MAX_NOISE-1)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            cpt, joinedFactor = joinFactorsByVariable(cpt, var)
        #if the factor has more than one uncon_var:
            num_var = len(joinedFactor.unconditionedVariables())
            if num_var > 1:
                temp = eliminate(joinedFactor, var)
                cpt.append(temp)
        joined_factor = joinFactors(cpt)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        self.normalize()
        keys = list(self.keys())
        dist = list(self.values())
        random_key = random.choices(keys, dist)[0]
        return random_key
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            temp = self.getObservationProb(observation, Pac_pos, pos, self.getJailPosition())
            new = self.beliefs[pos] * temp
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        time_elapsed = DiscreteDistribution()
        for pos in self.allPositions:
            prob = self.beliefs[pos]
            newPos = self.getPositionDistribution(gameState, pos)
            for pos in newPos.keys():
                time_elapsed[pos] += newPos[pos] * prob
        self.beliefs = time_elapsed
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        par_for_eachPos = self.numParticles//len(self.legalPositions)
        par_left = self.numParticles % len(self.legalPositions)
        for pos in self.legalPositions:
            if par_left != 0:
                self.particles.append(pos)
                par_left -= 1
            for i in range(par_for_eachPos):
                self.particles.append(pos)
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        temp = DiscreteDistribution()
        for par in self.particles:
            obs_prob = self.getObservationProb(observation, pac_pos, par, self.getJailPosition())
            if par in temp:
                temp[par] += obs_prob
            else:
                temp[par] = obs_prob
